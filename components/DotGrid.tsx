...
function hexToRgb(hexOrVar: string) {
  ...
  if (typeof document !== 'undefined' && hex.startsWith('var(')) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (typeof document !== 'undefined' && baseColor.startsWith('var(')) {
          const varName = baseColor.slice(4, -1).trim();
          fillStyle = getComputedStyle(document.documentElement).getPropertyValue(varName).trim();
        }
